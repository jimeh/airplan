(() => {
  ...
  (function() {
    ...
    function Z(j) {
      ...
      var y = Math.max.apply(null, W.map(function(G) {
        return G.number;
      }));
      if (y !== j.latest_revision)
      ...
      var m = q < y, qj = m ? "Revision " + q + " of " + y : "Revision " + q + " (Latest)", D = z.createElement("span");
      D.className = "revision-picker-label", D.textContent = qj, D.setAttribute("aria-hidden", "true");
      var w = z.createElement("select");
      w.setAttribute("aria-label", "Document revision"), W.forEach(function(G) {
        var B = z.createElement("option");
        B.value = G.safeURL || "", B.textContent = G.number === y ? "Revision " + G.number + " (Latest)" : "Revision " + G.number + " of " + y, B.selected = G.number === q, w.appendChild(B);
      }), w.addEventListener("change", function() {
        var G = w.selectedIndex;
        if (G < 0 || G >= W.length)
          return;
        window.location.assign(W[G].safeURL || "");
      }), _.replaceChildren(D, w), _.classList.add("is-picker"), _.classList.toggle("is-stale", m), z.body.classList.toggle("airplan-stale-revision", m);
    }
    if (V) {
    ...
    if (z.querySelectorAll(".viewtoggle button").forEach(function(j) {
      ...
      var H = Array.from(X.querySelectorAll('a[href^="#"]')), c = X.querySelector(".toc-list");
      if (c)
        if (I = z.createElement("dialog"), typeof I.showModal === "function") {
          ...
          var N = z.createElement("nav");
          N.className = "toc-dialog-nav", N.setAttribute("aria-label", "Table of contents"), N.appendChild(c.cloneNode(!0)), P.appendChild(f), P.appendChild(N), I.appendChild(P), O = z.createElement("button"), O.className = "toc-trigger", O.type = "button", O.tabIndex = -1, O.setAttribute("aria-label", "Open table of contents"), O.setAttribute("aria-controls", "toc-dialog"), O.setAttribute("aria-haspopup", "dialog"), O.setAttribute("aria-hidden", "true"), O.innerHTML = a, z.body.appendChild(O), z.body.appendChild(I), O.addEventListener("click", function() {
            I.showModal(), z.body.classList.add("toc-dialog-open"), x();
            ...
            if (z.body.classList.remove("toc-dialog-open"), x(), O.classList.contains("is-visible"))
              setTimeout(function() {
                O.focus();
              }, 50);
          }), N.querySelectorAll("a").forEach(function(q) {
